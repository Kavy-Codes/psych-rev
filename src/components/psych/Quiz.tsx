...
export function Quiz({ chapterRange }: Props) {
  ...
  const questions = useMemo(() =>
    startCh === 0 ? quizQuestions : quizQuestions.filter(q => q.chapter >= startCh && q.chapter <= endCh),
    [startCh, endCh]
  );
    ...
    <div className="flex flex-col h-full px-4 pt-2 pb-3 gap-2.5">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto no-scrollbar" key={currentIndex}>
        <div className="animate-fade-in">
          ...
          {answered && (
            <div className="mt-3 p-3 rounded-xl bg-indigo-500/5 border border-indigo-500/15 animate-slide-up">
      ...
      {answered && (
        <button
          onClick={next}
          className="w-full py-3 rounded-xl bg-indigo-600 text-white font-bold text-sm active:scale-95 transition-all shadow-lg shadow-indigo-500/20 shrink-0 animate-slide-up"
        >
